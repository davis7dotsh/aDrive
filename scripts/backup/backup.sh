#!/usr/bin/env bash
# a-drive nightly backup: R2 sync + Postgres dump + retention + manifest.
...
: "${DATABASE_URL:?DATABASE_URL is required (read-only Postgres role)}"
...
PG_DIR="${BACKUP_ROOT}/postgres/daily"
PG_MONTHLY_DIR="${BACKUP_ROOT}/postgres/monthly"
MANIFEST_DIR="${BACKUP_ROOT}/manifests"
LOG_DIR="${BACKUP_ROOT}/logs"
STATUS_FILE="${BACKUP_ROOT}/last-run.json"
LOG_FILE="${LOG_DIR}/backup-${STAMP_DAY}.log"

mkdir -p "${MIRROR_DIR}" "${TRASH_DIR}" "${PG_DIR}" "${PG_MONTHLY_DIR}" \
...
mv "${PG_TMP}" "${PG_EXPORT}"
log "postgres dump written: $(du -h "${PG_EXPORT}" | cut -f1) ${PG_EXPORT}"
...
# the dump digest — enough to audit a restore without the live account.
	...
	|| fail "manifest-enumeration"
{
	...
	printf ',"postgresDump":{"file":"%s","sha256":"%s"}' \
		"$(basename "${PG_EXPORT}")" \
		"$(shasum -a 256 "${PG_EXPORT}" | cut -d' ' -f1)"
	printf '}\n'
...
# --- 4. Retention -----------------------------------------------------------
# Daily dumps, manifests, logs, and deleted-object trash: 30 days.
# Monthly snapshots: 12 months.
find "${PG_DIR}" -name '*.sql.gz' -mtime +30 -delete
