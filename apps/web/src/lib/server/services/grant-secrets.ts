import { Context, Effect, Layer, Schema } from 'effect';
import { AppConfig } from '../config';
import { StorageError } from '../errors';
import { createObjectTtlCache } from '../isolate-cache';
import { PgSql } from '../pg';
import { CurrentOrg } from './current-org';
import {
	mintPrivateGrant,
	verifyPrivateGrant,
	type MintPrivateGrantOptions,
	type PrivateGrant,
	type VerifyPrivateGrantOptions
} from '../private-grant';

const PersistedSecretRow = Schema.Struct({
	content_grant_signing_key: Schema.String
});

const KEY_PATTERN = /^[A-Za-z0-9_-]{43}$/;

// The grant signing key is seeded once per instance and never rewritten, so
// reading it from Postgres on every request that mints or verifies a private
// link (each /link call plus each /t grant check) is wasted round trips.
// Cache it per isolate like the tag list and indexing status. The Postgres
// client is built per request, so the cache is keyed by a module-scope
// sentinel: there is exactly one database per deployment. If the database
// is ever reseeded, mint+verify stay consistent within an isolate, and a
// stale key on a handful of isolates self-heals on the next cache expiry.
const SIGNING_KEY_CACHE_TTL_MS = 5 * 60_000;
const signingKeyCacheKey = {};
const signingKeyCache = createObjectTtlCache<object, string>(
	SIGNING_KEY_CACHE_TTL_MS
);

// Grants are bound to the current org's content origin
// (`<slug>.<content domain>`): a dashboard request mints for its own org,
// a content request verifies for the org its host names. Verification
// additionally requires the request to arrive on exactly that origin.
type MintGrantInput = Omit<
	MintPrivateGrantOptions,
	'signingKey' | 'contentOrigin'
>;
type VerifyGrantInput = Omit<
	VerifyPrivateGrantOptions,
	'signingKey' | 'contentOrigin'
>;

export interface GrantSecretsShape {
	readonly mint: (
		input: MintGrantInput
	) => Effect.Effect<PrivateGrant, StorageError>;
	readonly verify: (
		input: VerifyGrantInput
	) => Effect.Effect<boolean, StorageError>;
}

export class GrantSecrets extends Context.Service<
	GrantSecrets,
	GrantSecretsShape
>()('app/GrantSecrets') {}

const randomSigningKey = () => {
	const bytes = new Uint8Array(32);
	crypto.getRandomValues(bytes);
	return btoa(String.fromCharCode(...bytes))
		.replaceAll('+', '-')
		.replaceAll('/', '_')
		.replace(/=+$/u, '');
};

const makeGrantSecrets = Effect.gen(function* () {
	const sql = yield* PgSql;
	const config = yield* AppConfig;
	const org = yield* CurrentOrg;

	const signingKey = Effect.gen(function* () {
		const cached = signingKeyCache.get(signingKeyCacheKey);
		if (cached) return cached;
		const candidate = randomSigningKey();
		yield* sql`
			INSERT INTO instance_secrets (
				id, content_grant_signing_key, created_at
			) VALUES (1, ${candidate}, ${new Date().toISOString()})
			ON CONFLICT (id) DO NOTHING
		`;
		const rows = yield* sql`
			SELECT content_grant_signing_key
			FROM instance_secrets
			WHERE id = 1
		`;
		const decoded = Schema.decodeUnknownOption(PersistedSecretRow)(rows[0]);
		if (
			decoded._tag !== 'Some' ||
			!KEY_PATTERN.test(decoded.value.content_grant_signing_key)
		) {
			return yield* Effect.fail(
				'The persisted content grant key is unavailable'
			);
		}
		signingKeyCache.set(
			signingKeyCacheKey,
			decoded.value.content_grant_signing_key
		);
		return decoded.value.content_grant_signing_key;
	}).pipe(
		Effect.mapError(
			(cause) =>
				new StorageError({
					operation: 'initialize content grant signing key',
					cause
				})
		)
	);

	return GrantSecrets.of({
		mint: Effect.fn('GrantSecrets.mint')(function* (input) {
			const contentOrigin = config.contentOriginFor(org.slug);
			const key = yield* signingKey;
			return yield* Effect.promise(() =>
				mintPrivateGrant({ ...input, contentOrigin, signingKey: key })
			);
		}),
		verify: Effect.fn('GrantSecrets.verify')(function* (input) {
			const contentOrigin = config.contentOriginFor(org.slug);
			const key = yield* signingKey;
			return yield* Effect.promise(() =>
				verifyPrivateGrant({ ...input, contentOrigin, signingKey: key })
			);
		})
	});
});

export const GrantSecretsLive = Layer.effect(GrantSecrets, makeGrantSecrets);
